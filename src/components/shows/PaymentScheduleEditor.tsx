...
export function PaymentScheduleEditor({ showId, cacheTotal: cacheProp, canEdit, onChanged }: Props) {
  ...
  const update = (idx: number, patch: Partial<ScheduleItem>) => {
    setItems((prev) => prev.map((it, i) => {
      if (i !== idx) return it;
      const next = { ...it, ...patch };
      // Recalcula valor a partir do percentual quando o usuário edita o %
  ...
  const addRow = () => {
    setItems((prev) => [
      ...prev,
      { ordem: prev.length, descricao: "", data_prevista: "", percentual: null, valor: 0, observacoes: "" },
    ]);
  };
            ...
            <li key={idx} className="rounded-md border p-3 space-y-2">
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-2">
                ...
                <div className="sm:col-span-1 flex items-end justify-end">
                  {canEdit && (
                    <Button size="icon" variant="ghost" onClick={() => removeRow(idx)} title="Remover">
      ...
      {canEdit && (
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-3 text-xs text-muted-foreground">
            <span>Soma %: <strong className={totalPercentual > 100 ? "text-destructive" : ""}>{totalPercentual.toFixed(2)}%</strong></span>
